...
| Browser → host round-trip | ✅ | `browser-verify.mjs` drives the real UI: creates a room, adds a member, reads both back, removes the member, deletes the room. One browser action therefore exercises the descriptor table, the gateway, the host validator, the durable domain and the JSON return path. `docs/images/room-manager.png` is that run's screenshot |
